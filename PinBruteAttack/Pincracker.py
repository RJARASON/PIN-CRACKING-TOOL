    ...
    while True:
        ...
        for guess in generate_all_pins(length):
            ...
            if targethash == hashedpin:
                print("\033[93m="*80)
                file=open(".\PinBruteAttack\RekPins.log",'a')
                ...
                notification.notify(
                    title="Pin Cracking Tool",
                    message="✅ PIN HAS BEEN FOUND.",
                    timeout=7
                )
                exit()
        length += 1 

except (Exception,KeyboardInterrupt,FileExistsError,FileNotFoundError):
    print("\n\n\033[91m[!] An unexpected error occured. This can happen by various reasons : \n\n > File used for storing pins is not found or doesn't exist \n > Program got interrupted by user \n > A bug occured in the program \n > Some modules or libraries are not install.\033[0m")
    exit()
